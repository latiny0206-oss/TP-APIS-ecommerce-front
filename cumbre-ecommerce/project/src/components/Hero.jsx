/* Cinematic full-bleed hero — primary CTA in pine, accent in alpenglow */

const HERO_IMAGES = {
  midnight: 'https://images.unsplash.com/photo-1486870591958-9b9d0d1dda99?auto=format&fit=crop&w=2000&q=80',
  ridge:    'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=2000&q=80',
  glacier:  'https://images.unsplash.com/photo-1454496522488-7a8e488e8606?auto=format&fit=crop&w=2000&q=80',
};

const Hero = ({ variant = 'midnight' }) => {
  const dispatch = useDispatch();
  return (
    <section className="relative grain overflow-hidden bg-rock">
      <div className="absolute inset-0">
        <img src={HERO_IMAGES[variant] || HERO_IMAGES.midnight} alt=""
             className="w-full h-full object-cover"
             onError={(e) => { e.currentTarget.style.display = 'none'; }} />
        <div className="absolute inset-0 bg-gradient-to-r from-rock via-rock/70 to-rock/20" />
        <div className="absolute inset-0 bg-gradient-to-t from-rock via-transparent to-rock/30" />
      </div>

      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10 pt-16 lg:pt-28 pb-24 lg:pb-36">

        {/* Eyebrow */}
        <div className="rise rise-d1 inline-flex items-center gap-3 mb-8 lg:mb-12 border border-ivory/20 bg-rock/40 backdrop-blur-sm px-4 py-2">
          <span className="inline-block h-1.5 w-1.5 rounded-full bg-alpenglow animate-pulse" />
          <span className="font-mono text-[11px] tracking-widest-2 uppercase text-ivory/90">
            20% OFF en colección de invierno · Código INVIERNO24
          </span>
        </div>

        {/* Headline */}
        <h1 className="rise rise-d2 font-display font-black uppercase leading-[0.85] tracking-tightest
                       text-[14vw] sm:text-[11vw] lg:text-[9.5rem] xl:text-[11rem] max-w-[12ch]">
          <span className="block text-ivory">Equípate</span>
          <span className="block text-ivory">para la</span>
          <span className="block text-alpenglow italic font-narrow font-medium tracking-tight normal-case text-[0.92em]">cima</span>
        </h1>

        {/* Sub + CTA */}
        <div className="rise rise-d3 mt-10 lg:mt-14 grid lg:grid-cols-[1fr_auto] gap-8 lg:gap-12 items-end max-w-6xl">
          <p className="text-ivory/75 text-lg lg:text-xl leading-relaxed max-w-xl">
            Rendimiento sin concesiones para las condiciones más extremas. Descubre nuestra nueva línea de equipamiento técnico diseñado para resistir.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 lg:gap-4">
            <Button variant="primary" size="lg"
              onClick={() => dispatch(navigate('catalog'))}
              iconRight={<IconArrowRight size={18} stroke={2.2}/>}>
              Ver catálogo
            </Button>
            <Button variant="ghost-dark" size="lg"
              onClick={() => dispatch(navigate('orders'))}
              iconRight={<IconArrowUpRight size={16}/>}>
              Mis expediciones
            </Button>
          </div>
        </div>

        {/* Bottom stats strip */}
        <div className="rise rise-d4 mt-16 lg:mt-24 border-t border-ivory/10 pt-6 grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-10">
          <StatCell label="Código activo"        value="INVIERNO24" mono accent />
          <StatCell label="Productos en stock"   value="1.482" />
          <StatCell label="Garantía de por vida" value="Toda la línea Pro" />
          <StatCell label="Envío express"        value="48 horas" />
        </div>
      </div>

      {/* Floating code chip */}
      <div className="hidden lg:flex absolute bottom-36 right-10 xl:right-16 flex-col items-end gap-2">
        <span className="font-mono text-[10px] tracking-widest-2 uppercase text-ivory/50">aplicado en checkout</span>
        <div className="flex items-center gap-3 bg-ivory text-rock px-5 py-3">
          <IconCheck size={16} stroke={2.6} className="text-pine" />
          <span className="font-mono font-bold tracking-widest-2 text-sm">INVIERNO24</span>
          <span className="text-rock/50 text-xs">−20%</span>
        </div>
      </div>
    </section>
  );
};

const StatCell = ({ label, value, mono, accent }) => (
  <div>
    <div className="font-mono text-[10px] tracking-widest-2 uppercase text-ivory/45 mb-1.5">{label}</div>
    <div className={`${mono ? 'font-mono' : 'font-narrow font-semibold'} ${accent ? 'text-alpenglow' : 'text-ivory'} text-base lg:text-lg`}>
      {value}
    </div>
  </div>
);

window.Hero = Hero;
