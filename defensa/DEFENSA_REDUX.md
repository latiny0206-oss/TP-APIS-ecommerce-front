...
**¿Y por qué `auth/initDone` aparece dos veces en Redux DevTools en dev?**
Es React StrictMode (activado en `main.jsx`), que en desarrollo monta cada componente dos veces a propósito para exponer efectos con side-effects no idempotentes. `AuthInit` (`App.jsx:52-71`) despacha desde un `useEffect`, así que el dispatch corre dos veces. La segunda pasa por el reducer pero devuelve el mismo estado — RDT lo marca como diff vacío. Se puede verificar corriendo `npm run build && npm run preview` (StrictMode no hace doble-invoke en producción). Las actions que nacen de eventos del usuario (`addToCart`, `updateQty`, `loginThunk`, etc.) NO se duplican ni en dev, porque no viven en un `useEffect`. Y donde el efecto sí tenía un fetch caro — el catálogo en `ProductsContext` — pusimos un guard con `useRef` para que StrictMode no lo dispare dos veces.
