...
|---|---|
| `user` / `isLoggedIn` | `loginThunk.fulfilled`, `registerThunk.fulfilled`, `sessionRestored`, `logout` |
...
|---|---|---|
| `sessionRestored` | Carga `user` desde localStorage, `isLoggedIn = true`, apaga `initializing` | `App.jsx:55` (`AuthInit`, al montar la app, si hay token + user guardados) |
| `initDone` | Solo apaga `initializing` (no había sesión guardada) | `App.jsx:57` (`AuthInit`) |
| `logout` | Resetea todo el slice al estado deslogueado | `Navbar.jsx:73`, `Perfil.jsx:35`, `AdminLayout.jsx:99` (botones de logout) y `App.jsx:63` (evento `auth:logout` que emite el interceptor ante un 401) |
...
| `setReturnTo` | Guarda la ruta pendiente | `Carrito.jsx:23` (quiso ir al checkout sin login) y `App.jsx:172` (`AccountGuard`) |
...
- **Efecto colateral**: emite `CustomEvent('auth:login', { detail: { id } })`, que `CartUserCheck` usa para limpiar el carrito si cambió el usuario.
...
### `registerThunk` (authSlice.js:21-37)
...
|---|---|---|
| `App.jsx:166` (`AccountGuard`) | `isLoggedIn`, `initializing` | Proteger `/cuenta/*`; muestra loader mientras `initializing` |
| `App.jsx:183` (`AdminGuard`) | + `user` | Exigir `user.rol === 'ADMIN'` para `/admin/*` |
...
- **"¿Cómo renuevan el token?"** — No se renueva: el JWT dura 24 h (`jwt.expiration=86400000` en el backend). Cuando vence, la primera llamada devuelve 401 → el interceptor de response borra localStorage y emite `auth:logout` → `AuthInit` (App.jsx:61-68) despacha `logout()` y navega a `/login`. Es un logout reactivo, no un refresh token.
...
- **El logout tiene dos mitades**: `authService.logout()` limpia localStorage (token, user, carrito) y `dispatch(logout())` limpia el store. En los tres botones de logout se llaman juntas; si preguntan por qué no alcanza una: una borra la persistencia, la otra el estado vivo.
