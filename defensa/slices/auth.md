...
| `initializing` | Empieza `true`. **En la práctica lo bajan siempre `sessionRestored` o `initDone`** (uno de los dos corre al montar `AuthInit`). `loginThunk.fulfilled`, `registerThunk.fulfilled` y `logout` también lo asignan a `false`, pero es defensivo — cuando corren, `initializing` ya vale `false`, así que en Redux DevTools esa parte de la action sale con diff vacío. Ver "puntos finos" abajo. |
...
- **¿Por qué `login` y `register` fulfilled también apagan `initializing`?** — Defensivo. Si por algún motivo llegara un login antes de que `AuthInit` corriera (no debería, pero es un race teórico), el slice queda consistente igual. Cuesta una asignación, evita una clase entera de bugs. **Importante para la defensa**: si abrís Redux DevTools, hacés login y buscás el cambio de `initializing` en la action `auth/login/fulfilled`, **no lo vas a ver** — el diff de ese campo sale vacío porque `AuthInit` ya lo bajó al montar la app (unos milisegundos antes de que el usuario pudiera tocar el submit). El reducer *asigna* `false`, pero como el valor previo ya era `false`, DevTools no muestra transición. Lo mismo aplica al reducer `logout` y a `register.fulfilled`. Es correcto que el código lo haga (blindaje) y es correcto que no se vea (idempotencia). Si en la defensa alguien pregunta "mostrame dónde baja `initializing` en el login", la respuesta es: *"no lo baja en la práctica, ya estaba en false; el que lo baja de verdad es `sessionRestored` o `initDone` al arrancar la app — la asignación en login/register/logout es defensiva por si alguna vez llegan sin que `AuthInit` haya corrido"*.
