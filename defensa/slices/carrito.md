...
- **Guard por rol**: `CartBackendSync` no hace nada si el usuario logueado tiene `rol === 'ADMIN'`. Ni vuelca al ocultar la pestaña ni recupera al iniciar sesión — el admin no tiene carrito propio. El evento `auth:login` viaja con `{ id, rol }` para que el handler pueda cortar sin tocar el store, y el `flushIfDirty` chequea el rol vía `useRef` sincronizado con `state.auth.user.rol`.
